...
- `make verify` at the repository root is the canonical local baseline (gofumpt check + `go vet` + short Go tests + current-platform build); run `make cross` when platform-specific compilation can be affected.
